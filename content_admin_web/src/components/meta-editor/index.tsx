...
import { getMetaUrl } from "@/utils/get-meta-url";
export interface JsonData {
  [key: string]: any;
}
const MetaEditor: FC<{ file: string; locale: string }> = ({ file, locale }) => {
  ...
  const openPage = useCallback(
    (key: string) => () => {
      ...
      const url = getMetaUrl(key);
      window.open(`${initUrl}${url}`, "_blank");
    },
    [hoastedURL, locale]
  );
          ...
          <div key={newPath}>
            <div>
              <Typography sx={{ textAlign: "left" }}>
                {splitCapitalizeAndRemoveExtension(key)}{" "}
                <IconButton edge="end" aria-label="url" onClick={openPage(key)}>
                  <OpenInBrowserIcon />
                </IconButton>
              </Typography>
            </div>
